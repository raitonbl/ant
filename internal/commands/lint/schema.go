...
func doLintSchemaSection(document *project.CliObject) ([]internal.Violation, error) {
...
func doLintSchemaFromSchemaSection(ctx *LintContext, keys []string, schema *project.Schema) ([]internal.Violation, error) {
	...
	if schema.TypeOf == nil {
		problems = append(problems, internal.Violation{Path: fmt.Sprintf("%s/type", ctx.prefix), Message: lint_message.REQUIRED_FIELD})
	}
